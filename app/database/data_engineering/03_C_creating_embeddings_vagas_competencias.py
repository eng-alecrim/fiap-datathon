...
async def fetch_documents_in_batches(
    ...
    batch = []
    total_processed = 0
    fist_field = text_field.split(".")[0]
    second_field = text_field.split(".")[1]

    try:
        async for doc in cursor:
            # Extract only what we need for processing
            item = {"id": str(doc["_id"]), "text": doc[fist_field][second_field]}
            batch.append(item)
...
async def main():
    ...
    text_field = "perfil_vaga.competencia_tecnicas_e_comportamentais"
